...
{
        u8 key_state = 0;
        static u16 K1_CNT = 0,K2_CNT = 0,K3_CNT = 0,K4_CNT = 0; 
        if(GPIO_ReadInputDataBit(K1_GPIO_PORT,K1_GPIO_PIN) == RESET)
        {
                K1_CNT++;
                if(K1_CNT > TEST_TIME)
                {
        ...
        if(GPIO_ReadInputDataBit(K2_GPIO_PORT,K2_GPIO_PIN) != SET)
        {
                ...
                if(K2_CNT > TEST_TIME)
                {
                        while(GPIO_ReadInputDataBit(K2_GPIO_PORT,K2_GPIO_PIN) == RESET)
                                ;
                        K2_CNT = 0;
                        key_state = 2;
                }
